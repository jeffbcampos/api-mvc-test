...
def confirmarEmail(token):
    try:
        response = db.session.query(Verificacao).filter_by(token=token, isValid=False).all()
        if response:
            db.session.query(Verificacao).filter_by(token=token).update({Verificacao.isvalid: True})
            db.session.commit()
